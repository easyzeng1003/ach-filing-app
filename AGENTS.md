...
- Form state persists in `localStorage` under key `ach-filing-forms-v2` (Zustand persist). Rows above **200** are not fully written（only header + 15 buffer rows）to avoid quota / UI jank. To prefill deterministically during manual testing you can seed this key and reload; a successful export shows a green toast "已產生 …txt（N 筆）" and downloads the file.
